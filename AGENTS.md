...
- The configuration center is disabled unless `configcenter.enabled=true`. When enabled, startup validation requires the configured Basic Auth username and password. Apply Basic Auth to the `/config-center` management page and management APIs, but not to `/config-center/api/runtime/*`; runtime endpoints use the namespace `X-API-Key` and still require the global configuration-center switch. Disabled configuration-center routes must return `404`, and Basic Auth must be deployed only behind HTTPS.
- Published configuration may be merged into Viper at `serve` startup through `[configload]` using either the trusted direct database source or the runtime HTTP API. Load only immutable published snapshots, fail startup on load errors, preserve Viper environment-variable precedence, and never allow a published payload to override the bootstrap roots `db`, `configcrypt`, `configcenter`, or `configload`. The database source uses local database and keyring settings; the HTTP source uses the namespace `X-API-Key`, rejects redirects, and enforces timeout and response-size limits.
