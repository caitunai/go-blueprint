...
- /storage/views : This directory is used to store HTML files that will be rendered by Golang. HTML files can be categorized into subfolders based on their functionality. Place basic layout files in the `/storage/views/layout` directory, like `/storage/views/layout/mylayout.html`, shared components in the `/storage/views/shared` directory, like `/storage/views/shared/toast.html`, and other specific pages can be categorized according to their functional modules，like `/storage/views/dashboard/report.mylayout.html`, `dashboard` is the module name, `report` is the function page, `mylayout` is your custom layout. To automatically reference the layout, page files need to follow the naming convention. The convention is as follows: If the base layout file is named `mylayout.html`, the functional page file should be named `feature.mylayout.html`, i.e., `featureName` + `layoutFileName`.
- /storage/ui : This directory is used to store programs related to front-end frameworks, such as independent programs for Vue, React, AlpineJS, TailwindCSS, and other front-end projects.
...
- Go page handlers should define a package-level entry constant, call `c.GetCSSJsFiles(entry)`, and pass `title`, `is_release`, `vite_origin`, `css_files`, and `js_files` into the template. Follow the existing `RecorderPage`, `CodecLabPage`, and `AudioArchivePage` pattern.
- HTML templates decide between development and release assets. In non-release mode, include `{{ .vite_origin }}/@vite/client` and the Vite source entry. In release mode, include the CSS and JS files resolved from `storage/ui/dist/manifest.json`.
- Development uses two servers: run the Go server in non-release mode and run `npm run dev` inside `/storage/ui`. The Vite dev origin defaults to `http://127.0.0.1:5173` and can be overridden with `ui.viteDevOrigin` in config.
