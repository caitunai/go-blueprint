...
- Tailwind CSS v4 does not automatically discover the Go HTML templates under `/storage/views` when the stylesheet is compiled from `/storage/ui`. Every Tailwind entry stylesheet must explicitly include the template source, for example `@source "../../../views/**/*.html";` after `@import "tailwindcss";` (adjust the relative path if the stylesheet depth changes). Do not rely on automatic source detection for classes used only in Go templates; otherwise component CSS may load while layout utilities such as `grid`, `flex`, spacing, widths, and responsive variants are silently omitted.
